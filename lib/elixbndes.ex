defmodule ElixBndes do
  ...
  """
  def get_fornecedores_by_nome_produto(nome_produto, pagina \\ 1)
      when is_bitstring(nome_produto) do
    case HTTPoison.get(
    ...
    case HTTPoison.post(
      ...
      {:ok, %{body: raw_body, status_code: _code}} ->
        ...
        %{
          ...
          pagina_atual: pagina,
          total_paginas: Kernel.round(total_produtos / 10) + 1
        }
  ...
  defp get_produtos(document) do
    document
    |> Floki.find("table[class=Tabela1]")
  ...
  defp get_quantidade_produtos_by_regex(nome_produto) do
    case HTTPoison.post(
           ...
           {:form,
            [
              {"chr_PalavraPesquisadaHidden", nome_produto},
              {"chr_PalavraPesquisada", nome_produto}
            ]},
           %{"Content-Type" => "application/x-www-form-urlencoded", "source" => "elixbndes"}
         ) do
      {:ok, %{body: raw_body, status_code: _code}} ->
        ...
        document
        |> Floki.find("a[id=qtdeprod_anchor5]")
        |> Floki.text()
        |> String.replace(~r/[^\d]/, "")
        |> String.to_integer()
